        ...
        for split, dialogues in data.items():
            with open("{}_{}.json".format(dialogue_len, split), "w") as fout:
                ...
                for dialogue in tqdm(dialogues, total=len(dialogues)):
                    ...
                    ctx_relation, rsp_idx = get_relation(ctx_spk, ctx_adr, rsp_adr)

                    d = {'context': ctx, 'relation_at': ctx_relation, 'ctx_spk': ctx_spk, 'ctx_adr': ctx_adr,
                         'answer': rsp, 'ans_idx': rsp_idx, 'ans_spk': rsp_spk, 'ans_adr': rsp_adr}
