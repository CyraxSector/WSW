# coding=utf-8
"""WSW fine-tuning runner on the downstream task of reply utterance selection."""
...
def parse_exmp(serial_exmp):
    input_data = tf.compat.v1.parse_single_example(serial_exmp,
                                                   features={
                                                       "ctx_id":
                                                           tf.compat.v1.FixedLenFeature([], tf.int64),
                                                       "rsp_id":
                                                           tf.compat.v1.FixedLenFeature([], tf.int64),
                                                       "input_sents":
                                                       ...
                                                       "speaker_ids":
                                                           tf.compat.v1.FixedLenFeature([FLAGS.max_seq_length], tf.int64),
                                                       "label_ids":
                                                           tf.compat.v1.FixedLenFeature([], tf.float32),
                                                   }
                                                   )
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, speaker_ids, labels, num_labels,
    ...
    with tf.compat.v1.variable_scope("loss"):
        output_layer = tf.compat.v1.layers.dropout(output_layer, rate=0.1, training=is_training)
        ...
        mean_loss = tf.reduce_mean(losses, name="mean_loss") + sum(
...
def run_test(epoch, op_name, sess, training, accuracy, prob, pair_ids, saver, dir_path):
    ...
    try:
        ...
        accuracy, precision, recall, f1, loss = metrics.classification_metrics(results)
        print('Accuracy: {}, Precision: {}, Recall: {}, F1: {}, Loss: {}'.format(accuracy, precision, recall, f1, loss))
        ...
        print(
            'MAP (mean average precision: {}, MRR (mean reciprocal rank): {}, Top-1 precision: {}, Num_query: {}'.format(
